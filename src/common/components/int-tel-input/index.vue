...
{
  "en-US": {
    "filterInputPlaceholder": "Search for countries and regions",
    "searchNoResult": "search no result"
  },
  "zh-CN": {
    "filterInputPlaceholder": "搜索国家和地区",
    "searchNoResult": "搜索没有结果"
  }
}
</i18n>
<template lang="jade">
  .int-tel-input-wrapper.clear-float
        ...
        .scrollbar__wrap
          ...
          ul.scrollbar(ref="scrollbar")
            li.options.no-result(v-show="Object.keys(calculatePhoneData).length === 0")
              span(v-text="$t('searchNoResult')")
            li.options.clear-float(
              v-for="(item, key) in calculatePhoneData",
              :class="{'selected': countryAbbr === item.code}",
              @click.stop="selectOptionHandle(item, key)",
              :key="item.code"
            )
              span.int-countries-flag(:class="item.code")
...
<script>
  let phonesData = {
    ...
    as: { code: 'as', name: 'American Samoa', dialCode: 1, example: '(684) 733-1234' },
    ...
    ai: { code: 'ai', name: 'Anguilla', dialCode: 1, example: '(264) 235-1234' },
    // ag: { code: 'ag', name: 'Antigua and Barbuda', dialCode: 1268, example: '(268) 464-1234' },
    ag: { code: 'ag', name: 'Antigua and Barbuda', dialCode: 1, example: '(268) 464-1234' },
    ...
    bs: { code: 'bs', name: 'Bahamas', dialCode: 1, example: '(242) 359-1234' },
    ...
    bb: { code: 'bb', name: 'Barbados', dialCode: 1, example: '(246) 250-1234' },
    ...
    bm: { code: 'bm', name: 'Bermuda', dialCode: 1, example: '(441) 370-1234' },
    ...
    vg: { code: 'vg', name: 'British Virgin Islands', dialCode: 1, example: '(284) 300-1234' },
    ...
    ky: { code: 'ky', name: 'Cayman Islands', dialCode: 1, example: '(345) 323-1234' },
    ...
    dm: { code: 'dm', name: 'Dominica', dialCode: 1, example: '(767) 225-1234' },
    ...
    jm: { code: 'jm', name: 'Jamaica', dialCode: 1, example: '(876) 210-1234' },
    ...
    ms: { code: 'ms', name: 'Montserrat', dialCode: 1, example: '(664) 492-3456' },
    ...
    mp: { code: 'mp', name: 'Northern Mariana Islands', dialCode: 1, example: '(670) 234-5678' },
    ...
    kn: { code: 'kn', name: 'Saint Kitts and Nevis', dialCode: 1, example: '(869) 765-2917' },
    // lc: { code: 'lc', name: 'Saint Lucia', dialCode: 1758, example: '(758) 284-5678' },
    lc: { code: 'lc', name: 'Saint Lucia', dialCode: 1, example: '(758) 284-5678' },
    ...
    vc: { code: 'vc', name: 'Saint Vincent and the Grenadines', dialCode: 1, example: '(784) 430-1234' },
    ...
    sx: { code: 'sx', name: 'Sint Maarten', dialCode: 1, example: '(721) 520-5678' },
    ...
    tt: { code: 'tt', name: 'Trinidad and Tobago', dialCode: 1, example: '(868) 291-1234' },
    ...
    tc: { code: 'tc', name: 'Turks and Caicos Islands', dialCode: 1, example: '(649) 231-1234' },
    ...
    vi: { code: 'vi', name: 'U.S. Virgin Islands', dialCode: 1, example: '(340) 642-1234' },
  ...
  export default {
    ...
    computed: {
      selectedInput () {
        ...
        return currentCountry ? `${currentCountry.code.toUpperCase()} +${currentCountry.dialCode}` : ''
      },
      calculatePhoneData () {
        if (!this.filterString && !this.filterLetter) return phonesData

        let filter = this.filterString || `^${this.filterLetter}`
        const arr = this.filterString ? ['code', 'name', 'dialCode'] : ['name']
    ...
    methods: {
      ...
      keyDown (e) {
        let list = Object.values(this.calculatePhoneData)
        let index = list.findIndex(item => {
          return item.code === this.countryAbbr
        })
        switch (e.keyCode) {
          case 13:
            this.visible = false
          ...
          case 38:
            ...
            this.$emit('countryChange', list[index - 1])
            break
          case 40:
        ...
        this.$nextTick(this.setScrollIntoView)
      },
      ...
      scrollIntoView (container, selected) {
        ...
        this.$nextTick(() => {
          const containerPaddingTop = _getStyle2Number(container, 'padding-top')
          const containerPaddingBottom = _getStyle2Number(container, 'padding-bottom')
          const containerOffsetTop = container.offsetTop + containerPaddingTop
          ...
          if (top < viewRectTop + containerOffsetTop) {
            container.scrollTop = top - containerOffsetTop
          } else if (bottom > viewRectBottom + containerOffsetTop - containerPaddingBottom) {
            container.scrollTop = bottom - containerClientHeight - containerOffsetTop + containerPaddingBottom
          }
        })
      },
    ...
    mounted () {
      let currentCountry = phonesData[this.countryAbbr]
      if (!this.countryAbbr && this.countryCode) {
        currentCountry = Object.values(phonesData).find(item => {
          return item.dialCode == this.countryCode
        })
      }
  ...
  function _getStyle2Number (el, property) {
    return ~~getComputedStyle(el).getPropertyValue(property).replace(/[^\d]/g, '')
  }
</script>

<style lang="stylus">
.int-tel-input .el-input__inner
  height: 38px
</style>
